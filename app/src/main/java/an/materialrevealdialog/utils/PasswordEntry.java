...
package an.materialrevealdialog.utils;
...
import an.materialrevealdialog.R;
...
public class PasswordEntry extends TextInputEditText {
    ...
    public PasswordEntry(Context context) {
        super(context);
        show_password = getTransformationMethod() instanceof PasswordTransformationMethod;
    }
    ...
    public void setText(CharSequence text, BufferType type) {
        ...
        if (isMasked != show_password) {
            show_password = isMasked;
    ...
    static class MaskMorphDrawable extends Drawable {
        ...
        MaskMorphDrawable(Context context, TextPaint textPaint,
            ...
            fastOutSlowIn = Utils.getFastOutSlowInInterpolator(context);
        }
    ...
    static class PasswordCharacter {
        ...
        void draw(Canvas canvas, TextPaint paint, CharSequence password,
            ...
            // draw the character
            canvas.save();
            float textScale = Utils.lerp(1f, textToMaskScale, progress);
            ...
            paint.setAlpha((int) Utils.lerp(alpha, 0, progress));
            // vertically move the character center toward/from the mask center
            canvas.drawText(password, index, index + 1,
                    x, Utils.lerp(0f, textOffsetY, progress) / textScale, paint);
            canvas.restore();

            // draw the mask
            canvas.save();
            float maskScale = Utils.lerp(maskToTextScale, 1f, progress);
            ...
            paint.setAlpha((int) Utils.lerp(0, alpha, progress));
            // vertically move the mask center from/toward the character center
            canvas.drawText(PASSWORD_MASK, 0, 1, x, -Utils.lerp(textOffsetY, 0f, progress), paint);
            canvas.restore();
